...
Push-Location $WEBRTC_DIR\src
  # WebRTC Debugビルド x64
  Exec { gn gen $BUILD_DIR\debug_x64 --args="treat_warnings_as_errors=false rtc_use_h264=true ffmpeg_branding=`"Chrome`" rtc_include_tests=false rtc_build_tools=false rtc_build_examples=false is_component_build=false use_rtti=true use_custom_libcxx=false" }
  Exec { ninja -C "$BUILD_DIR\debug_x64" }

  # WebRTC Releaseビルド x64
  Exec { gn gen $BUILD_DIR\release_x64 --args="is_debug=false treat_warnings_as_errors=false rtc_use_h264=true ffmpeg_branding=`"Chrome`" rtc_include_tests=false rtc_build_tools=false rtc_build_examples=false is_component_build=false use_rtti=true strip_debug_info=true symbol_level=0 use_custom_libcxx=false" }
